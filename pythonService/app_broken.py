...
def after_request(response):
        """Add CORS headers to all responses"""    response.headers.add('Access-Control-Allow-Origin', '*')
...
def health_check():
        """Health check endpoint"""    return jsonify({"status": "ok", "message": "Service is running"}), 200
...
def train_template():
        """Train a new template from an image"""    if 'image' not in request.files:
...
def verify_document():
        """Verify uploaded document against template database"""    try:
...
def serve_visualization(filename):
        """Serve visualization images"""    visualizations_dir = os.path.join(TEMP_DIR, "visualizations")
...
def upload_template():
        """Upload a new template document"""    # Check for uploaded file
...
def generate_pdf():
        """Handle verification data submission"""    try:
...
if __name__ == '__main__':
        # Start server    print("Starting document verification service on port 5000...")
